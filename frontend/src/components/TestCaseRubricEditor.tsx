...
type Props = {
  value?: StepRubric;
  onChange?: (v: StepRubric) => void;
};

const labels: Record<string, string> = ru.testCase.criterionLabels;

export default function TestCaseRubricEditor({ value, onChange }: Props) {
  const rubric = value ?? { criteria: [], pass_percent: 60, free_text: "" };

  const update = (patch: Partial<StepRubric>) => {
  ...
  const items = CRITERION_CATALOG.map((cat) => {
    const c = rubric.criteria.find((x) => x.id === cat.id) ?? {
      id: cat.id,
      enabled: true,
    ...
    return {
      ...
      children: (
        <Space direction="vertical" style={{ width: "100%" }}>
          <div>
            <Typography.Text>{ru.testCase.rubricWeight}</Typography.Text>
            <InputNumber
              min={0}
              max={100}
              value={c.weight}
          ...
          <Input.TextArea
            rows={2}
            placeholder={ru.testCase.rubricHintPlaceholder}
            value={c.hint}
            disabled={!c.enabled}
  ...
  return (
    <Space direction="vertical" style={{ width: "100%" }} size="middle">
      <Typography.Text type="secondary">{ru.testCase.rubricIntro}</Typography.Text>
      <Collapse items={items} />
      <div>
        <Typography.Text>{ru.testCase.passPercent}</Typography.Text>
        <InputNumber
          min={0}
          max={100}
      ...
      <div>
        <Typography.Text>{ru.testCase.rubricFreeText}</Typography.Text>
        <Input.TextArea
          rows={2}
          value={rubric.free_text}
          onChange={(e) => update({ free_text: e.target.value })}
          placeholder={ru.testCase.rubricFreeTextPlaceholder}
        />
      </div>
    </Space>
  );
